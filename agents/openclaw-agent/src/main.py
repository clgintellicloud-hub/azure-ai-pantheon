# OpenClaw Agent HTTP wrapper for local mocks and real runtime commands.
...
app = FastAPI(title="OpenClaw Agent Wrapper")
...
async def execute_runtime(request: TaskRequest) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="OpenClaw runtime timed out") from exc
    except FileNotFoundError as exc:
        raise HTTPException(status_code=500, detail="OpenClaw runtime command not found") from exc
    ...
    if process.returncode != 0:
        raise HTTPException(
            status_code=502,
            detail={
                "message": "OpenClaw runtime failed",
